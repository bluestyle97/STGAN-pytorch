...
class STGANAgent(object):
    def __init__(self, config):
        ...
        self.D = Discriminator(self.config.image_size, len(self.config.attrs), self.config.d_conv_dim, self.config.d_fc_dim, self.config.g_layers)
    ...
    def train(self):
        ...
        x_sample, c_org_sample = next(data_iter)
        ...
        for i in range(self.current_iteration, self.config.max_iters):
            ...
            out_adv, out_att = self.D(x_real)
            d_loss_real = - torch.mean(out_adv)
            d_loss_att = self.classification_loss(out_att, label_org)

            # compute loss with fake images
            attr_diff = c_trg - c_org
            x_fake = self.G(x_real, attr_diff).detach()
            out_adv, out_att = self.D(x_fake)
            d_loss_fake = torch.mean(out_adv)

            # compute loss for gradient penalty
            alpha = torch.rand(x_real.size(0), 1, 1, 1).to(self.device)
            x_hat = (alpha * x_real.data + (1 - alpha) * x_fake.data).requires_grad_(True)
            out_adv, _ = self.D(x_hat)
            d_loss_gp = self.gradient_penalty(out_adv, x_hat)

            # backward and optimize
            d_loss_adv = d_loss_real + d_loss_fake + self.config.lambda_gp * d_loss_gp
            d_loss = d_loss_adv + self.config.lambda1 * d_loss_att
            ...
            scalars['D/lr'] = self.lr_scheduler_D.get_lr()[0]
            ...
            if (i + 1) % self.config.n_critic == 0:
                ...
                out_adv, out_att = self.D(x_fake)
                g_loss_adv = - torch.mean(out_adv)
                g_loss_att = self.classification_loss(out_att, label_trg)

                # reconstruction
                x_rec = self.G(x_real, c_org - c_org)
                g_loss_rec = torch.mean(torch.abs(x_real - x_rec))

                # backward and optimize
                g_loss = g_loss_adv + self.config.lambda2 * g_loss_att + self.config.lambda3 * g_loss_rec
                ...
                scalars['G/lr'] = self.lr_scheduler_G.get_lr()[0]
            ...
            if self.current_iteration % self.config.sample_step == 0:
                with torch.no_grad():
                    ...
                    for c_trg_sample in c_trg_sample_list:
                        attr_diff = c_trg_sample.to(self.device) - c_org_sample.to(self.device)
                        x_fake_list.append(self.G(x_sample, attr_diff))
    ...
    def test(self):
        ...
        for i, (x_real, c_org) in enumerate(tqdm_loader):
            ...
            for c_trg in c_trg_list:
                attr_diff = c_trg - c_org
                x_fake_list.append(self.G(x_real, attr_diff))
            x_concat = torch.cat(x_fake_list, dim=3)
            result_path = os.path.join(self.config.result_dir, 'result_{}.jpg'.format(i + 1))
